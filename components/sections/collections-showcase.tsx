  ...
  return (
    <section id="collections" className="paper-grain relative py-16 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <SwipeRow
          count={publishedCollections.length}
          gridClassName="grid-cols-2 lg:grid-cols-3"
          className="mt-12 sm:mt-14"
        >
          {publishedCollections.map((c, i) => (
            <SwipeItem key={c.id}>
              <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-warm-sm transition-all duration-[240ms] hover:-translate-y-1.5 hover:shadow-warm">
                <div className="relative aspect-[5/4] overflow-hidden sm:aspect-[4/3]">
                  ...
                  <span className="absolute left-3 top-3 rounded-full bg-black/65 px-2.5 py-1 text-xs uppercase tracking-[0.18em] text-ivory backdrop-blur-sm">
                ...
                <div className="flex flex-1 flex-col p-5 sm:p-6">
                  ...
                  <p className="mt-3 text-base leading-relaxed text-muted-foreground sm:text-sm">{c.intro}</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {c.subHampers.map((s) => (
                      <span
                        key={s.id}
                        className="rounded-full border border-border bg-secondary/50 px-3 py-1 text-sm text-secondary-foreground"
                      >
                        {s.name}
                      </span>
                    ))}
                  ...
                  <a
                    ...
                    className="mt-auto inline-flex items-center gap-2 pt-6 text-sm font-medium text-primary-ink transition-colors hover:text-maroon"
                  >
